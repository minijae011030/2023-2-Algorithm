...
void searchAndFixAfterInsertion(AVLTree* node);	// 노드 삽입 후균형검사를 수행하고 불균형이 있으면 개조를 통해 높이균형 속성을 회복
...
AVLTree* reduceExternal(AVLTree* node);	
void searchAndFixAfterRemoval(AVLTree* node);	// 노드 삭제 후 균형 검사를 수행하고 불균형이 있으면 개조를 통해 높이 균형 속성을 회복
int removeElement(AVLTree* node, int key);	// 노드 삭제
...
void main() {
	...
	while (1) {
		...
		case 's':	// s가 입력되면 노드 탐색
			...
			break;
		case 'd':	// d가 입력되면 노드 삭제
			scanf("%d", &key);
			element = removeElement(root, key);

			if (element == key) printf("%d\n", element);
			else printf("X\n");
...
void searchAndFixAfterInsertion(AVLTree* node) {
	// 노드 삽입 후 균형검사를 수행하고 불균형이 있으면 개조를 통해 높이균형 속성을 회복
	AVLTree* x, * y, * z;
...
AVLTree* reduceExternal(AVLTree* node) {
	AVLTree* nodeParent, * nodeSibling, * nodeGrandParent;
	nodeParent = node->parent;	// 노드의 부모노드
	nodeSibling = sibling(node);	// 노드의 형제 노드

	if (nodeParent->parent == NULL) {	// 부모노드가 루트노드인 경우
		root = nodeSibling;	// 루트노드 변ㄴ경
		nodeSibling->parent = NULL;
	}
	else {	// 부모노드가 루트노드가 아닌 경우
		nodeGrandParent = nodeParent->parent;	
		nodeSibling->parent = nodeGrandParent;
		if (nodeParent == nodeGrandParent->leftChild) nodeGrandParent->leftChild = nodeSibling;
		else if (nodeParent == nodeGrandParent->rightChild) nodeGrandParent->rightChild = nodeSibling;
	}
...
void searchAndFixAfterRemoval(AVLTree* node) {
	// 노드 삭제 후 균형 검사를 수행하고 불균형이 있으면 개조를 통해 높이 균형 속성을 회복
	AVLTree* x, * y, * z, * b;
	if (node == NULL) return;

	z = node;
	while (updateHeight(z) && isBalanced(z)) {
		if (z->parent == NULL) return;	// 루트에 접근하면 함수 종료
		z = z->parent;	// 부모노드로 접근
	}
...
int removeElement(AVLTree* node, int key) {
	// 노드 삭제
	AVLTree* w = treeSearch(node, key);
	if (isExternal(w)) return -1;	// 외부 노드에 다다르면 noSuchKey 리턴

	AVLTree* z = w->leftChild, * zs, * y;
...
AVLTree* restructure(AVLTree* x, AVLTree* y, AVLTree* z) {
	...
	// x, y, z의 중위순회 방문순서의 나열을(a, b, c)라 하자.
	// x, y, z의 부트리들 가운데 x, y, z를 루트로 하는 부트리를 제외한 4개의 부트리들의 중위순회 방문순서 나열을 (T0, T1, T2, T3)이라 하자 
...
void expandExternal(AVLTree* node) {
	// 노드 확장
	AVLTree* leftnode = (AVLTree*)malloc(sizeof(AVLTree));
	AVLTree* rightnode = (AVLTree*)malloc(sizeof(AVLTree));
