...
AVLTree* insertItem(AVLTree* tree, int key);	// 현재 트리에 key를 저장한 새 노드를 삽입하고 searchAndFixAfterInsertion 함수 호출
AVLTree* treeSearch(AVLTree* node, int key);	// 현재 트리에서 키 key를 저장한 노드를 반환

AVLTree* searchAndFixAfterInsertion(AVLTree* root, AVLTree* node);	// 균형검사를 수행하고 불균형이 있으면 개조를 통해 높이균형 속성을 회복
bool updateHeight(AVLTree* node);	// 노드 node의 높이를 (필요하면) 갱신한 후 갱신 여부를 반환
bool isBalanced(AVLTree* node);	// 노드 node의 높이균형 여부를 반환
AVLTree* restructure(AVLTree** root, AVLTree* x, AVLTree* y, AVLTree* z);	// 3-노드 개조를 수행한 후 (갱신된) 3-노드의 루트를 반환
...
AVLTree* insertItem(AVLTree* node, int k) {
	// 현재 트리에 key를 저장한 새 노드를 삽입하고 searchAndFixAfterInsertion 함수 호출
	AVLTree* p = treeSearch(node, k);

	if (isInternal(p)) return NULL;
	else {
		p->key = k;
		expandExternal(p);
		//printf("searchAndFixAfterInsertion 전: %d\n", node->key);
		AVLTree* q = searchAndFixAfterInsertion(node, p);
		if (q) node = q;
		//printf("searchAndFixAfterInsertion 후: %d\n", node->key);
	}
...
AVLTree* searchAndFixAfterInsertion(AVLTree* root, AVLTree* node) {
	...
	if (node->parent == NULL) return NULL;	// 해당 노드가 루트노드이면 함수 종료

	z = node->parent;	// 해당 노드의 루트 노드를 z로 설정
	while (updateHeight(z) && isBalanced(z)) {
		if (z->parent == NULL) return NULL;	// 루트노드에 접근하면 반복문 종료
		z = z->parent;	// 조건에 만족하는 동안 부모노드로 접근
	}
	if (isBalanced(z)) return NULL;	// 루트노드에서 균형이 맞으면 함수 종료
	...
	//printf("searchAndFixAfterInsertion 내부 전: %d\n", root->key);
	root = restructure(&root, x, y, z);	// x, y, z에 대한 노드 개조 수행
	//printf("searchAndFixAfterInsertion 내부 후: %d\n", root->key);
	return root;
}
...
AVLTree* restructure(AVLTree** root, AVLTree* x, AVLTree* y, AVLTree* z) {
